...
print(summary(ml))
## tol
mlTol <- maxLik( llf, start = startVal, tol=1e-2)
print(summary(mlTol))
mlTolC <- maxLik(llf, start=startVal, control=list(tol=1e-2))
...
## reltol
mlRelTol <- maxLik( llf, start = startVal, reltol=1e-2)
print(summary(mlRelTol))
mlRelTolC <- maxLik(llf, start=startVal, control=list(reltol=1e-2))
