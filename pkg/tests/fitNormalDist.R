...
mlSANNCand <- maxLik( llf, start = startVal, method = "SANN",
   cand = function(x,fnOrig=NULL,gradOrig=NULL,hessOrig = hess)c(rnorm(1,x[1]),rnorm(1,x[2])) )
summary( mlSANNCand )
...
mlSannConCand <- maxLik( llf, start = startVal, constraints = eqCon,
   method = "SANN", cand = function(x,fnOrig=NULL,gradOrig=NULL,hessOrig=NULL)c(rnorm(1,x[1]),rnorm(1,x[2])) )
