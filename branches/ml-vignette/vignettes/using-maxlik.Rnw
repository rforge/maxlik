...
can use the correct convenience functions, such as \verb|summary| or
\verb|logLik|.  It should also be stressed that \maxlik
\emph{maximizes}, not \emph{minimizes} the function.
...
elliptical contours.  In this example the Newton-Raphson algorithm
...
$\vec{\epsilon}$ is standard normal disturbance term:
<<>>=
...
observations are independent, and may not work with data, such as time
series, where this is not true.
...
instance, in case our function values are very large, we may want to
...
maximization algorithms, together with likelihood-related methods,
such as
...
vector.
We demonstrate
this with the first example of the vignette example where the task was
to estimate the
