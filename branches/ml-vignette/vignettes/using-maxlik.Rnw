...
\verb|maxLik|'s arguments' names.  In the latter case it
...
correct if we do not work with a likelihood function.  The arguments
...
message, we learn about ``successful convergence''.


\subsection{Testing condition numbers}
\label{sec:testing-condition-numbers}

Non-linear optimizers perform best in regions that level sets
(contours) are
roughly circular.
If an area that is fairly flat with contours extremely elongated
ellipses,
...
In such case one may want to test the
...
essentially multicollinear.  In the linear regression
...
character as \verb|c("beta2", "beta4")| if \verb|start| has names.  We demonstrate
...
case to 1-dimensional.  Hence the inference for $\mu$ is still correct.
