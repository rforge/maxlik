...
for a version without \verb|dnorm|):
<<>>=
...
\verb|grad|:
<<>>=
m <- maxLik(nSSE, grad=grad, start=c(0,0,0))
summary(m, eigentol=1e-15)
@
...
<<hessianExample>>=
m <- maxLik(nSSE, grad=grad, hess=hess, start=c(0,0,0))
summary(m, eigentol=1e-15)
@ 
...
<<>>=
compareDerivatives(nSSE, grad, t0=c(0,0,0))
...
X <- cbind(x1, x2, x3)
y <- X %*% c(1, 1, 1) + rnorm(100)
m <- maxLik(nSSEA, start=c(x1=0, x2=0, x3=0))
                           # give start values a bit off
summary(m)
@
