...
sub-optimal solution, or completely fail to converge if numeric gradients are too noisy.  Needless to
...
(contours) are
roughly circular.  However, using data in a very different
...
fails to converge when relying on numeric derivatives.
...
last parameter estimate are also way off from the correct value $(1,
1, 1)$.

% not using matrix notation

We create three variables,
$x_{1}$, $x_{2}$ and $x_{3}$ at very different scale, and compute $y$ as
\begin{equation}
  \label{eq:different-scale-linear-regression}
  y_{i} = x_{1i} + x_{2i} + x_{3i} + \epsilon_{i}
\end{equation}
Hence the correct parameter values are $\beta_{1} = \beta_{2} =
\beta_{3} = 1$.  Denote $\vec{\beta} = (\beta_{1}, \beta_{2}, \beta_{3})$.
We optimize the negative of the sum-of-squared
errors
\begin{equation}
  \label{eq:linear-regression-sse}
  \mathit{SSE}(\vec{\beta}) =
  \sum_{i}
  (y_{i} - x_{1i}\beta_{1} - x_{2i}\beta_{2} - x_{3i}\beta_{3})^{2},
\end{equation}
this is equivalent to maximizing likelihood in case of
...
m <- maxNR(SSE, start=c(x1=0, x2=0, x3=0))
                           # give start values a bit off
summary(m)
@
As one can see, the optimizer gets stuck and returns completely wrong
values.  The reason here is that the loss function (ESS) is of
extremely elongated shape, and the finite difference approximation is
too crude to correctly compute the curvature.
In this particular case one can easily solve the problem by
re-scaling the variables, or
choosing a more robust optimizer.\footnote{As the loss function is
  quadratic, Newton-Raphson should provide the correct solution
  in a single
  iteration only, given the gradient and Hessian are correct.
}
But
this is not always possible, in particular complex
functions in high dimensions may contain regions of unknown shape,
flat in one direction and extremely curved in another,
where the algorithm cannot easily proceed.

Here we solve the problem by introducing analytic derivatives
instead.  The gradient of $\mathit{SSE}$ is
\begin{equation}
  \label{eq:dsse-dbeta}
  \pderiv{\vec{\beta}}\mathit{SSE}(\vec{\beta}) =
  (\vec{e}^{\transpose} \vec{x}_{1},
  \vec{e}^{\transpose} \vec{x}_{2}, 
  \vec{e}^{\transpose} \vec{x}_{3})
\end{equation}
where $\vec{e} = (e_{1}, e_{2}, \dots, e_{N})$ and $e_{i} = y_{i} -
x_{1i}\beta_{1} - x_{2i}\beta_{2} - x_{3i}\beta_{3}$ is vector of the
residuals. 


In such case we may learn more about the problem by testing the
condition number of Hessian matrix.  \verb|condiNumber| runs such
test variable-by-variable:
<<>>=
condiNumber(hessian(m))
@ 
Condition number of the first column of Hessian ($1\times1$ matrix) is always
one.  When we add the second column corresponding to $x_{2}$, the
...
In particular, what helps here to locate the problem is to observe
how the condition
number increases when we keep adding columns.  This allows us to see which
...
In a similar fashion, one may want to test for multicollinearity in the
dataset or the design matrix.  In a complex case it is easy to add too
many fixed effects or include combinations of variables that are
essentially multicollinear.  In the linear 
regression problem the design matrix is just
\begin{equation}
  \label{eq:ols-design-matrix}
  \mat{X} =
  \begin{pmatrix}
    \vec{x}_{1} & \vec{x}_{2} & \vec{x}_{3}
  \end{pmatrix},
\end{equation}
i.e. the three data vectors stacked next to each other.  We test the
condition numbers of it by
<<>>=
X <- cbind(x1, x2, x3)
condiNumber(X)
@
...
loglikA <- function(theta) {
   ...
   gradient <- numeric(2)
   gradient[1] <- sum(x - mu)/sigma^2
   gradient[2] <- -N/sigma + sse/sigma^3  # re-use sse here
...
(contours) are
roughly circular.
In a fairly flat area with contours that are extremely elongated
ellipses,
the performance will degrade and the algorithms
may completely fail to converge.  The highly elliptical level sets may
be caused by different factors, below we demonstrate a simple linear regression example with
very different variable range.  We create three variables,
$x_{1}$, $x_{2}$ and $x_{3}$ at very different scale, and compute $y$ as
\begin{equation}
  \label{eq:different-scale-linear-regression}
...
Newton-Raphson optimizer fails to represent it correctly because
the numeric gradient is too noisy.
In this particular case one can easily solve the problem by
re-scaling the variables, introducing analytic derivatives, or
choosing a more robust optimizer.\footnote{As the loss function is
  quadratic, Newton-Raphson should provide the correct solution
  in a single
  iteration only, given the gradient and Hessian are correct.  This is
  apparently not the case here as numeric gradient cannot correctly
  handle the extreme curvature along $x_{3}$.
}
But
this is not always possible, in particular complex
functions in high dimensions may contain regions of unknown shape,
...
\begin{itemize}
\item example with Hessian
\item analytic gradient example: use OLS with very unequal scale
\item stress that maximization, not minimization
\item condinumber: do collinearity, not different scales
\end{itemize}
