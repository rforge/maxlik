...
data to find out the best possible set of parameters.  \emph{Maximum
  likelihood} (ML) method finds such combination of parameters' value that
...
dataset, given that our probability model is correct.  These desirable
properties, and relative simplicity of the method, have made ML one of
the favorite statistical estimators.
