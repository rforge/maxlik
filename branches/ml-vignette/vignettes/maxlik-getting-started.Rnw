...
random numbers (these numbers are distributed as the ``bell curve'') the
...
\begin{equation}
  ...
  f(x_{1}) =
  \lim_{\delta\to0}
  \frac{\Pr(X \in [x_{1} - \delta/2,\, x_{2} + \delta/2])}{\delta}.
\end{equation}
...
$x_{2}$ after
semicolon.  Now we just  $\mu$ by just finding such a $\mu$ value that maximizes the
expression. 
