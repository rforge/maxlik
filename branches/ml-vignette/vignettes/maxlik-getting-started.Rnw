...
% \VignetteIndexEntry{Maximum Likelihood: Introduction}
...
The previous example is instructive but it does not really
applicable.  The problem there is that we wrote the probability model
as standard normal density with unknown location $\mu$.  However, in
practice we hardly ever know if we are dealing with standard normal,
more likely both the location and the standard deviation $\sigma$ are
unknown.  So in order to make the previous example applicable, we need
to include the unknown standard in the model.

The normal density function with standard deviation $\sigma$ is
\begin{equation}
  \label{eq:normal-pdf}
  f(x; \mu, \sigma) =
  \frac{1}{\sqrt{2\pi}}
...
come to the well-known intuitive results: the best estimator for location $\mu$ is
the sample mean, and for variance $\sigma^{2}$ the sample variance.
...
need all the parameters submitted as a single vector.  So we can write
...
single vector $\vec{\theta}$.  We have also
dropped dependency on data in the notation altogether but
remember that in practical applications log-likelihood will always
depend on data.  This notation can converted to computer code almost
verbatim, just when actually calculating log-likelihood inside of
the function, one has to extract the parameters from $\vec{\theta}$
vector. 

Example: fathers and sons


\subsection{Final Example: Linear Regression}
\label{sec:linear-regression}

Now when we have the main tools in place, it is rather easy to extend
the example above to a real statistical model.

Let us analyze the \emph{CO2} dataset (in package \emph{datasets}).
This describes \COii uptake (variable \emph{uptake}) by different grasses in
...
problem, we need again a parametric probability model.  Assume that
the disturbance term $\epsilon$ is normally distributed with mean 0
and (unknown) variance $\sigma^{2}$.  So we can
...
\begin{equation}
  ...
  \beta_{1} \cdot \mathit{conc}_{i}
\end{equation}
and finally plug this into~\eqref{eq:co2-epsilon-loglik}:
\begin{multline}
...
\item We write the log-likelihood as a function of parameters
  ...
  still a parameter, one we are usually not interested in (sometimes
...
\item The function definition itself is just sum of log-likelihood
  ...
  not observe those directly, we express those through the regression
  equation in~\eqref{eq:co2-single-loglik}.
\end{itemize}
Note that the first term on the right hand side does not depend on the
parameters and hence we can ignore it for the optimization, and write
the function in a simpler form:
\begin{equation}
  ...
  \frac{(
    ...
    - \mu)^{2}}{\sigma^{2}}.
\end{equation}
This is the definition we can easily code and estimate.


\verb|summary(lm(uptake ~ conc + plant, data=CO2))|
