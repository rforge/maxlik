...
probability that the random outcome $X$ falls into the narrow  width
$\delta$ interval
...
\end{equation}
In the example on the Figure the values around $x_{1}$ and less likely
...
\begin{figure}[ht]
  ...
  \caption{Standard normal probability density (thick line).  While
...
\item write the probability as $\likelihood(\mu)$, likelihood function of parameter
  $\mu$;
...
is just a multiplicative factor, and a positive one
...
``$1/2$'' is a positive factor that can be ignored as well.
...
the sample average, and for variance $\sigma^{2}$ it is the sample variance.
...
summary(m)
@ 
Indeed, our guess was close.  It is in general advisable to find good
starting values, but in this simple case it is of little importance.
...
data. 
Next, we can express $\epsilon_{i}$ from linear regression model~\eqref{eq:co2-regression}:
\begin{equation}
...
log-likelihood based on the previous iteration.  While humans can look at
