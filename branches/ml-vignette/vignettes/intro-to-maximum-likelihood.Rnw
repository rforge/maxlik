...
over the parameters of interest.  We start with a discrete data example
to explain what is
likelihood and how it is related to probability.  The following
sections cover continuous data,
...
probabilities?  Obviously, in case of coin there are only two
...
success to be heads below, but you can as well choose tails).
...
and let the algorithm to find the best possible $p$ from there:
<<>>=
library(maxLik)
...
and hence we write the likelihood as
\begin{equation}
...
\end{itemize}
Besides of these two, the formula now includes $\sigma$
and sums over all observations.
...
\COii concentration (variable \emph{conc}) in air.  We can
...
follow~\eqref{eq:two-normal-loglik} and write for a single observation
\begin{equation}
...
the argument list of $\loglik$.  Otherwise we now have random outcome
$\epsilon_{i}$ instead of $x$, so we write $\epsilon_{i}$ in place of
data. 
We do not know $\epsilon_{i}$ but
we can express using linear regression model~\eqref{eq:co2-regression}:
\begin{equation}
...
We can visualize the task by plotting the log-likelihood for
...
log-likelihood value compared to
the previous iteration.  While humans can look at
...
just a white fog and the ground is covered with exactly as white snow.  But you
can still feel where the ground goes up and so you can still go
...
can easily mean days of work.  The second approach, numeric gradient,
...
likely it is that it gets stuck in a wrong place or just wanders
cluelessly around.  If this is the case, you may see how the algorithm
gets slow, refuses to converge (returns the ``maximum number of
