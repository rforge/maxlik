...
concept of likelihood, or need a quick intuitive brush-up.
...
consult the other vignette, ``Maximum Likelihood Estimation with
\maxlik'' instead.

Maximum Likelihood (ML) in its core is maximizing the \emph{likelihood}
over the parameters of interest.  We start with explaining what is
likelihood and how it is related to probability in case of discrete
random variables.  The following sections cover continuous random
variables, multiple parameters as parameter vectors, and end with an
example of linear regression.  The final section discusses the basics
