...
The basic method is simple and intuitively, it is essentially just a
slow climb in the
...
``observations'', typically arranged as the rows of the design matrix $\mat{X}$, that
are independent of each other.

The overview above does not specify how to compute the gradient
$\vec{g}(\vec{\theta}_{0})$ in terms of which observations $i$
...
$\mat{X}$ and $\vec{y}$), and the sum can be
...
where the gradient is computed on a
single observation only with
\begin{equation}
...
different performance issues.  In particular,
...
nature of SGA.  Next, we describe the API and explain the differences
...
probably will in most cases), but one can run the optimizer without
it.
...
add the gradient name \texttt{grad=} as otherwise the gradient will be
treated as the objective function!

If provided, the function should accept two (or more) arguments: the
first is the numeric parameter vector, and the second, named
...
will be done if the objective function is provided but the gradient
isn't),
this
...
The gradient function must have the parameter vector as the first formal argument, and it
also must have argument \texttt{index}, a numeric index for the
...
\item Number of epochs (control option \texttt{iterlim}):
  ...
  using the minibatches.  This is analogous 
\item Norm of gradient, analogous to \texttt{maxNR}.  However, in case
  ...
  option \texttt{gradtol}) is set to $0$ by default.  If interested,
...
mainly discuss the momentum and Adam, the basic gradient-only based optimization technique was
...
where one commonly relies on stochastic optimization, it a simple and
well-understood
...
the squared norm at 10,000, i.e. the gradient norm won't grow above
100: 
<<>>=
...
\texttt{storedValues} methods.  The former returns a $(R+1) \times K$
...
to $0.001$ using the \texttt{SG\_learningRate} control.  Note while we
are maximizing negative loss, we plot positive loss.
...
@
Note that as the optimizer only uses training data, the \texttt{nObs} argument
...
regression, the slow-down is very large.  Hence we lower the number of
...
learning rate down to $2\cdot10^{-7}$:
...
\texttt{val}.  If the algorithm stops early, the stored values are
left uninitialized (\texttt{NA}-s), hence we use \texttt{na.omit} in
order to only include the actual values in the final
\texttt{val}-vector.
We allow the algorithm to run for 1000 epochs, but as we now have
...
We did not encounter early stopping here, all loops run through 200
epochs.  The figure contains three flat areas, corresponding to the
burn-in of the Adam method.  After 30 or so initial epochs it proceeds
in a steady pace toward lower loss. 
