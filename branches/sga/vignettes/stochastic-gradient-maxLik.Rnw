...
stock gradient ascent (including momentum), and \emph{Adam}.
\emph{Adam}, the basic gradient-only based optimization technique was
explained in Section~\ref{sec:stochastic-gradient-ascent}.

It is easy and intuitive to extend this method with momentum.  As
implemented in \texttt{maxSGA}, the momentum $\mu$ is incorporated
...
region with stable gradient.  As a downside, it may overshoot a
minimum.  But this behavior makes
momentum-equipped SGA less prone of getting stuck in a local optimum.

\emph{SGA} optimizer can be invoked with \texttt{maxSGA}.

Adaptive Moment or \emph{Adam} \citep[p. 301]{goodfellow+2016DL} adapts the learning rate by
...
Both \texttt{maxSGA} and \texttt{maxAdam} are designed similar to
\texttt{maxNR}, and expect the objective function \texttt{fn},
gradient \texttt{grad}, and
Hessian function \texttt{hess}, and parameter start values \texttt{start}.  As these optimizers
only need gradient, one can leave out both \texttt{fn} and
\texttt{hess}.  The Hessian is only used to compute final Hessian, if
requested by the user.  As SGA methods are typically used in contexts
where Hessian is not needed, or even infeasibly slow and large, by
defalt the algorithms do not return Hessian matrix and hence do not
use the \texttt{hess} function even if provided.
Finally, unlike the traditional opitmizers, stochastic optimzers need
...
the gradient with the given value while keeping it's direction.  Clip
...
value, we have to supply an objective function to maxSGA.  We compute MSE on the same dataset as
<<>>=
...
As the next step, we use Adam optimizer:
