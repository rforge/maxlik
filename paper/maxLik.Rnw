...
It is mainly a wrapper function
...
library( maxLik )
@
Now, we can use the \code{maxLik} function
...
\end{align}
These gradients are calculated by following function:
<<>>=
...
\footnote{%
\code{all.equal} considers two elements as unequal
...
If a function to calculate gradients but no function to calculate Hessians
is provided by the user,
the Hessians are calculated numerically by the function
\code{numericHessian}.
However, the user can use the argument \code{hess}
to provide a function that calculates the Hessian analytically.
The elements of the Hessian of the log likelihood function
...
\end{align}
They can be calculated using the following function:
<<>>=
...
@
This function can be provided to the \code{maxLik} function
by the argument \code{hess}:
<<>>=
...
\code{"NR"} for the Newton-Raphson algorithm, but it can also be
\code{"BHHH"} for Berndt-Hall-Hall-Hausman \cite{berndt74},
\code{"BFGS"} for Broyden-Fletcher-Goldfarb-Shanno
\cite{broyden70,fletcher70,goldfarb70,shanno70},
\code{"NM"} for Nelder-Mead \cite{nelder65}, or
\code{"SANN"} for simulated-annealing \cite{belisle92}).
The \code{maxLik} function delegates the actual optimisation
to the corresponding functions
\code{maxNR}, \code{maxBHHH}, \code{maxBFGS}, \code{maxNM}, and
\code{maxSANN}, respectively.
