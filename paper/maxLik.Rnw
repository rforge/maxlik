...
optimization algorithms, different ways of calculation
variance-covariance matrices, and reporting the results.  The examples
...
developers, implementing ML estimators of specific models.
...
basic and more advanced usage of the package, and
...
Gradient matrix by observations is accepted as well.  If the user does
not provide gradients, the gradients are computed based on the
implementation of the numeric derivatives in the package.
...
\item At the end, we send inform \code{maxNR} algorithm of our
  dicision by seting respective attributes to log-likelihood.  Two
  attributes are used: \code{constPar} informs the algorithm that
  corresponding parameters in the parameter vector must further be treated as
  fixed; and \code{newVal} (which contains two components -- indices and
  values) informs the algorithm which parameters have new values.  It
  is possible to fix parameters without providing new values by
