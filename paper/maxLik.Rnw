...
\begin{abstract}
  ...
  statistical environment ``\textsl{R}''.  The package is essentially a unified
...
Although \R \citep{r-project09} has built-in optimization
...
Hessian).  The \code{maxBHHH} function belongs to this layer as well,
...
\code{constrOptim} in the package \pkg{stats}.  The \code{maxLik} function
...
\code{coef} method and standard errors by the \code{std} method:
<<code:coef-std>>=
coef(mle)
std(mle)
@ 
...
\code{numericHessian}.\footnote{Only the Newton-Raphson 
...
as in the case of numerical gradients, numerical Hessians may turn out to
be slow and unreliable.


\subsection{Optimisation Methods}

The \code{maxLik} function allows to select between five
...
providing a corresponding gradient function (see below) or providing a
...
In the latter case, numeric gradients are used to calculate the Hessian.
...
A Gradient matrix with gradients of individual observations, is accepted as well.
If the user does
...
Although \R includes general-purpose optimizers such as \code{nlm}
and \code{optim} for a long time,
