...
Berndt-Hall-Hall-Hausman type algorithm \citep{berndt74}, popular for ML problems.  In
...
for instance arguments \code{grad} and \code{hess} for the
Nelder-Mead method---%
...
expectation of the outer product of the gradient (score) vector, i.e.
\begin{equation}
H \approx - \frac{1}{N} \sum_{i=1}^N \;
   \frac{ \partial \log L_i ( \beta ) }{ \partial \beta } \;
   \left( \frac{ \partial \log L_i ( \beta ) }{ \partial \beta } \right)^\top ,
\end{equation}
where $\partial \log L_i ( \beta ) / \partial \beta$ is the gradient
of the log-likelihood contribution of the $i$th observation
