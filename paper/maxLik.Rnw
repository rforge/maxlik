...
\begin{abstract}
  This paper describes the \pkg{maxLik} for \textsl{R} statistical
  environment.  The package is essentially a unified wrapper interface
  to various optimization routines with interface, offering easy
  access to likelihood-specific features like standard errors, or
  information equality (BHHH method).  More advanced features of the
  opimization algorithms, such as forcing the value of a particular
  parameter be fixed, is also supported.
...
which is suitable for maximum likelihood (ML) estimation.  The package
implements a flexible multi-purpose Newton-Raphson type optimization
routine \code{maxNR}.  This is used for basis of \code{maxBHHH}, a
...
\pkg{stats}) is wrapped in a unified way (functions
\code{maxBFGS}, \code{maxNM}, and
\code{maxSANN}).  

The package is designed in two layers: first is the optimization
(maximization) layer: all the maximization routines are designed to be
directly usable for being easy to switch.  All the main parameters
have identical names and similar order (however, method-specific
parameters may vary).  These functions can be used for different types
of optimization tasks, both related and not related to the
likelihood.  They return an object of class \code{maxim} including
both estimated parameters and various diagnostics information.
...
computing the variance-covariance matrix based on estimated Hessian).
The \code{maxBHHH} function belong to this layer as well, being
essentially a call for \code{maxNR} using information equality as the
approximate Hessian matrix.  A new class \code{maxLik} is added to the
maximization object for automatic selection of the ML-related methods.
...
is aware of the constraints and is able to select a suitable method,
...
is a function with the (same) name \code{maxLik}.  It mostly a wrapper
for different optimization routines with a few additional features,
useful for ML estimations.  This includes interpretation of Hessian as
the inverse of negative variance-covariance matrix, and methods for
printing and accessing results.
...
The easiest way is to use the capabilities of the function
\code{dnorm}: 
<<>>=
...
derivatives may turn out to be costly to compute and noisy, which may
...
analytic derivatives, or switch to a more robust estimation method,
such as Nelder-Mead, which does not require gradient.
...
where each element corresponds to an individual observation.  This
...
different.  This is because different ways of calculating the Hessian
...
observation is supplied by the \code{grad} function.
<<>>=
...
the temperature-related parameters for \code{maxSANN}.  Below, we
...
<<code:activePar>>=
summary(maxLik(logLikFun, start=c(mu=0, sigma=2), activePar=c(TRUE, FALSE)))
@ 
...
automatically during the computations.  This may be useful if a large
...
algorithms do not work well at such circumstances.  We demonstrate it
...
value).  The results look the following:
<<code:mix1>>=
summary(m1 <- maxLik(logLikMix, start=c(rho=0.5, mu1=0, mu2=0)))
@ The estimates replicate the true parameters with high precision
(although $\mu_{1}$ and $\mu_{2}$ are swapped).  However, let us now
...
<<code:mix2>>=
summary(m2 <- maxLik(logLikMix, start=c(0.5, 0, 0)))
@ 
The estimation does not converge, although it approaches the correct
point of the parameter space: mixture of 100\% normal with mean 0 and
0\% with mean 1.  Note also that the Hessian of the model at the
estimated parameter value is not
negative definite.
If the estimation is done by hand, it would be easy
to fix the third parameter as in the example above.  However, this may
not be a suitable approach if we perform computations on a large
number of samples automatically.  In that case the logLik function has
to signal the \code{maxNR} routine that the parameter is now fixed.
