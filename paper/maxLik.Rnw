...
\R \citep{r-project09} has built-in optimization algorithms since it's
early days.  The first general-purpose ML framework (function
\code{mle} in package \pkg{stats4}) was added in 2003, and an
extension (\code{mle2} in package {bbmle}) in 2007.  However, both of
these packages are based on a general-purpose optimizer \code{optim},
which does not include an option to use Newton-Raphson, and in
particular, its variant Berndt-Hall-Hall-Hausman optimizer
