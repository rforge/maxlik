...
The \R package
\pkg{maxLik} \citep{r-maxlik-0.7} is intended to fill this gap.%
\footnote{The other
  ML-related packages offer capabilities, missing in
  \code{maxLik} (for instance, likelihood profiling).  This may
  be suboptimal from the user's persepective who might prefer a single,
...
for the actual optimization.  Additionally, a version of BFGS
optimizer, \code{maxBFGSR}, is provided by the package, which,
unlike \code{optim}-based methods, is implemented solely in \R.%
\footnote{\code{maxBFGSR} supplies more debugging information compared
...
(defined in the companion package \code{miscTools}):
<<code:coef-std>>=
coef(mle)
stdEr(mle)
@ 
...
to compute the gradient value in the likelihood function itself (this
may be a lot faster under certain circumstances).  The computed
gradient value may be added to the likelihood value as the attribute
``gradient'', analogously as for the function \code{nlm} of the
\code{stats} package.
...
Hessian value, instead of supplying the function, this can be done by 
setting the attribute ``hessian'' to the computed Hessian value.
...
\footnote{The user can force \code{maxLik} to use BHHH for
...
user may want to consider signaling the \code{maxNR} routine that the
...
Ott Toomet gratefully aknowledges financial support from 
