...
optimizer.  In particular, its variant variant 
Berndt-Hall-Hall-Hausman optimizer
\citep{berndt74}, is very popular for ML problems.  The package
...
routine \code{maxNR}.  This is used as the basis of \code{maxBHHH}, a
...
example: we estimate parameters of normal distribution based on a
random sample.
...
The BHHH method replaces the Hessian by the negative of the
expectation of the outer product of the gradient (score) vector.  Note
that this method is only valid while maximizing log-likelihood and
hence it is usually not included in general-purpose optimizers.  In
case of independent observations, the expectation can be approximated
by the sample average of the outer product of the score vectors of
individual observations.  Hence the algorithm needs gradient
...
done via arguments \code{activePar} or \code{fixedPar} of \code{maxNR}.  It is a logical
...
retaining their initial value.  Parameters can be kept fixed
by either specifying the free ones (by argument \code{activePar}), or
fixed ones (argument \code{fixedPar}).  Both of these arguments must be index vectors, if
both are specified, only the latter is used (and a warning issued).
So, as $\sigma$
...
automatically to a fixed constant during the computations when using \code{maxNR} optimizer.
...
to treat $\varrho$ as fixed as in the example above.  However, this may
