...
outer product of the gradient matrix $J = [ J_{ij} ]$, i.e.
$J \, J^\top$,
where $J_{ij} = \partial \log L_i ( \beta ) / \partial \beta_j$ is the gradient
of the log-likelihood contribution of the $i$th observation
with respect to the $j$th parameter ($\beta_j$).
Note
