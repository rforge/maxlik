...
include the \code{ml} command in \textsf{stata} and \textsf{GAUSS}
\code{maxlik} library.

Although \R \citep{r-project09} has had built-in optimization
algorithms  since it's early days, a comprehensive environment for ML
estimation has been missing until recently.  The package \pkg{maxLik}
\citep{r-maxlik-0.5} is intended to fill this gap.  The package can be
used both by end-users, developing their own statistical methods, and by package
developers, implementing ML estimators for specific models.
...
The focus on this paper is the maximum likelihood related
usage of the package, the other features (including numerical
...
Bernt-Hall-Hall-Hausman type optimizer \citep{berndt74}, popular for ML problems.  In
...
as the way to approxime the Hessian matrix.  A new class \code{maxLik} is added
...
variance-covariance matrix (just a warning is printed).  As the
constrained optimization should still be considered as experimental,
we refer the reader to the documentation of the package for examples.
...
of the estimated parameters, \code{std} for calculation standard
...
\code{coef}, and standard errors by \code{std} function:
<<code:coef-std>>=
coef(mle)
std(mle)
@ 
...
for instance, the argument \code{hess} for the Berndt-Hall-Hall-Hausman method.
...
order to calculate the expecation, the algorithm needs gradient
vectors by individual observations.  This can be achieved either by
providing corresponding gradient function (see below) or providing a
vector of individual observation-specific likelihood values by the
log-likelihood function itself.  In the latter case, those values are
used to compute corresponding numeric gradients.
...
where each column represents gradient according the the respective
...
following example uses a single summed log-likelihood from the
...
\pkg{stats}.  In order to retain compatibility with ``BHHH'' method,
...
Gradient matrix, based on individual observations, is accepted as well.  If the user does
not provide gradients, the gradients are computed numerically.

To give the reader a taste, we give an example using
...
\code{maxLik} function supports a variety of other arguments, most of
which are passed to the selected optimizer.  Among the most imporant
ones is \code{print.level} which controls the output of debugging
information (0 produces no debugging output, larger number produce
more). 
...
\subsection{Fixed parameter values}
...
``non-active'' parameters are treated as known constants, always
retaining their initial value.  So, as $\sigma$
...
\begin{itemize}
\item First, while changing the fixed parameters run-time, we have to
  keep track of the process.  This is why we introduce \code{freePar}
  \emph{outside} the function itself, as it have to retain it's value over
...
<<code:mix3>>=
summary(m <- maxLik(logLikMix1, start=c(rho=0.5, mu1=0, mu2=0.1)))
@ 
...
Although the general-purpoze optimizers, such as \code{nlm} function,
have existed in \R for a long time, 
...
a popular optimzation method which is available only for
likelihood-type porblems.
